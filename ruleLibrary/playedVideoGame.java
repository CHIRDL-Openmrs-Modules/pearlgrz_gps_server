/********************************************************************
 Translated from - playedVideoGame.mlm on Thu Mar 18 15:02:17 EDT 2010

 Title:  play video game Question
 Filename:  playedVideoGame.mlm
 Version: 1.0
...
public class playedVideoGame implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify school related problems. , , singleChoice";
	}
	...
	public String getTitle(){
		return "play video game Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result playedVideoGame=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Played a video game").within(Duration.days(-1)).last());
			resultLookup.put("playedVideoGame",playedVideoGame);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result playedVideoGame = (Result) resultLookup.get("playedVideoGame");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Played a video game".length();
				value=userVarMap.get("Played a video game");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Played a video game".endsWith("_value"))
				{
					variable = "Played a video game".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Played a video game".endsWith("_date"))
				{
					variable = "Played a video game".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Played a video game") != null){
						value = resultLookup.get("Played a video game").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Played a video game");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you play video game?");
		actions.add("¿Sabía usted el juego de vídeo?@Spanish");
	}
