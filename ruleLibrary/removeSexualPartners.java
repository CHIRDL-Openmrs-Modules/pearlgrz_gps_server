/********************************************************************
 Translated from - removeSexualPartners.mlm on Fri Mar 19 18:37:18 EDT 2010

 Title:  sexual partner related Question
 Filename:  removeSexualPartners.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  remove sexual partners. , , multiSelect , , partnerOptions
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class removeSexualPartners implements Rule, DssRule{
	...
	public String getPurpose(){
		return "remove sexual partners. , , multiSelect , , partnerOptions";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result removeSexualPartners=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Remove sexual partners").within(Duration.days(-1)).last());
			resultLookup.put("removeSexualPartners",removeSexualPartners);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result removeSexualPartners = (Result) resultLookup.get("removeSexualPartners");
		...
		if(removeSexualPartners.exists()){
			return false;
		}
			return true;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Remove sexual partners".length();
				value=userVarMap.get("Remove sexual partners");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Remove sexual partners".endsWith("_value"))
				{
					variable = "Remove sexual partners".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Remove sexual partners".endsWith("_date"))
				{
					variable = "Remove sexual partners".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Remove sexual partners") != null){
						value = resultLookup.get("Remove sexual partners").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Remove sexual partners");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Please select sexual partners to remove from history?");
	}
