/********************************************************************
 Translated from - timeForThirdSmokingWeed.mlm on Thu Mar 18 15:47:18 EDT 2010

 Title:  somking weed related Question
 Filename:  timeForThirdSmokingWeed.mlm
 Version: 1.0
...
public class timeForThirdSmokingWeed implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result answered1=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Starting-time for the Third smoking weed since last survey").within(Duration.days(-1)).last());
			resultLookup.put("answered1",answered1);
			Result answered2=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Ending-time for the Third smoking weed since last survey").within(Duration.days(-1)).last());
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))&&
			(!Input2.isNull()&&Input2.toString().equalsIgnoreCase("true"))){
				varLen = "Starting-time for the Third smoking weed since last survey".length();
				value=userVarMap.get("Starting-time for the Third smoking weed since last survey");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Starting-time for the Third smoking weed since last survey".endsWith("_value"))
				{
					variable = "Starting-time for the Third smoking weed since last survey".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Starting-time for the Third smoking weed since last survey".endsWith("_date"))
				{
					variable = "Starting-time for the Third smoking weed since last survey".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Starting-time for the Third smoking weed since last survey") != null){
						value = resultLookup.get("Starting-time for the Third smoking weed since last survey").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Starting-time for the Third smoking weed since last survey");
				}
				...
				logicService.eval(patient, "storeObs",parameters);
								varLen = "Ending-time for the Third smoking weed since last survey".length();
				value=userVarMap.get("Ending-time for the Third smoking weed since last survey");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Ending-time for the Third smoking weed since last survey".endsWith("_value"))
				{
					variable = "Ending-time for the Third smoking weed since last survey".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Ending-time for the Third smoking weed since last survey".endsWith("_date"))
				{
					variable = "Ending-time for the Third smoking weed since last survey".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).getResultDate().toString();
					}
				}
				...
				{
					if (resultLookup.get("Ending-time for the Third smoking weed since last survey") != null){
						value = resultLookup.get("Ending-time for the Third smoking weed since last survey").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Ending-time for the Third smoking weed since last survey");
				}
	...
	public void initAction() {
		...
		actions.add("When was the Third smoking weed?");
	}
