/********************************************************************
 Translated from - doNotApply.mlm on Thu Mar 18 11:17:18 EDT 2010

 Title:  do not apply
 Filename:  doNotApply.mlm
 Version: 1.0
...
public class doNotApply implements Rule, DssRule{
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "not applicable".length();
				value=userVarMap.get("not applicable");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("not applicable".endsWith("_value"))
				{
					variable = "not applicable".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("not applicable".endsWith("_date"))
				{
					variable = "not applicable".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("not applicable") != null){
						value = resultLookup.get("not applicable").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","not applicable");
				}
				varLen = "Yes".length();
