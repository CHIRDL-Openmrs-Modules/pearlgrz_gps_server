/********************************************************************
 Translated from - addGeneralPartner1.mlm on Fri Mar 19 18:27:25 EDT 2010

 Title:  general partner related Question
 Filename:  addGeneralPartner1.mlm
 Version: 1.0
...
public class addGeneralPartner1 implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result generalPartner1=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("General partner1").within(Duration.days(-1)).last());
			resultLookup.put("generalPartner1",generalPartner1);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result generalPartner1 = (Result) resultLookup.get("generalPartner1");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "General partner1".length();
				value=userVarMap.get("General partner1");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("General partner1".endsWith("_value"))
				{
					variable = "General partner1".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("General partner1".endsWith("_date"))
				{
					variable = "General partner1".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("General partner1") != null){
						value = resultLookup.get("General partner1").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","General partner1");
				}
