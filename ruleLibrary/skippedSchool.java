/********************************************************************
 Translated from - skippedSchool.mlm on Thu Mar 18 15:02:18 EDT 2010
...
public class skippedSchool implements Rule, DssRule{
	...
	public String getTitle(){
		return "Skipped School Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result skippedSchool=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Skipped school").within(Duration.days(-1)).last());
			resultLookup.put("skippedSchool",skippedSchool);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result skippedSchool = (Result) resultLookup.get("skippedSchool");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Skipped school".length();
				value=userVarMap.get("Skipped school");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Skipped school".endsWith("_value"))
				{
					variable = "Skipped school".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Skipped school".endsWith("_date"))
				{
					variable = "Skipped school".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Skipped school") != null){
						value = resultLookup.get("Skipped school").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Skipped school");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you skip school?");
		actions.add("¿Te faltan a la escuela?@Spanish");
	}
