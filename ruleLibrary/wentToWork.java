/********************************************************************
 Translated from - wentToWork.mlm on Thu Mar 18 11:22:19 EDT 2010
 ...
 Validation : research
 Purpose:  Identify work related problems.
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class wentToWork implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify work related problems.";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Went to work".length();
				value=userVarMap.get("Went to work");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Went to work".endsWith("_value"))
				{
					variable = "Went to work".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Went to work".endsWith("_date"))
				{
					variable = "Went to work".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Went to work") != null){
						value = resultLookup.get("Went to work").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Went to work");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you go to work today?");
		actions.add("¿Vas a ir a trabajar hoy?@Spanish");
	}
