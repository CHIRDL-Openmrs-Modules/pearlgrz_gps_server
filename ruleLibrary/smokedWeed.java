/********************************************************************
 Translated from - smokedWeed.mlm on Thu Mar 18 11:27:20 EDT 2010
...
public class smokedWeed implements Rule, DssRule{
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Smoked weed".length();
				value=userVarMap.get("Smoked weed");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Smoked weed".endsWith("_value"))
				{
					variable = "Smoked weed".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Smoked weed".endsWith("_date"))
				{
					variable = "Smoked weed".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Smoked weed") != null){
						value = resultLookup.get("Smoked weed").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Smoked weed");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you smoke weed?");
		actions.add("¿Has fumado hierba?@Spanish");
	}
