/********************************************************************
 Translated from - listenedToMusic.mlm on Thu Mar 18 15:02:19 EDT 2010

 Title:  Listened to music Question
 Filename:  listenedToMusic.mlm
 Version: 1.0
...
public class listenedToMusic implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify interests related problems. , , singleChoice";
	}
	...
	public String getTitle(){
		return "Listened to music Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result listenedMusic=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Listened to music").within(Duration.days(-1)).last());
			resultLookup.put("listenedMusic",listenedMusic);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result listenedMusic = (Result) resultLookup.get("listenedMusic");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Listened to music".length();
				value=userVarMap.get("Listened to music");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Listened to music".endsWith("_value"))
				{
					variable = "Listened to music".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Listened to music".endsWith("_date"))
				{
					variable = "Listened to music".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Listened to music") != null){
						value = resultLookup.get("Listened to music").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Listened to music");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you listen to music recently?");
		actions.add("¿Sabía usted escuchar música últimamente?@Spanish");
	}
