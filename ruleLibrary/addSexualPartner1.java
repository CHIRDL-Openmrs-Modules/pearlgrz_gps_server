/********************************************************************
 Translated from - addSexualPartner1.mlm on Fri Mar 19 18:27:20 EDT 2010

 Title:  sexual partner related Question
 Filename:  addSexualPartner1.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  add new sexual partners. , , singleInput
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class addSexualPartner1 implements Rule, DssRule{
	...
	public String getPurpose(){
		return "add new sexual partners. , , singleInput";
	}
	...
	public String getTitle(){
		return "sexual partner related Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result interactedWithSexualPartners=context.read(
			...
			resultLookup.put("interactedWithSexualPartners",interactedWithSexualPartners);
			Result newSexualPartners=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("New sexual partners to add").within(Duration.days(-1)).last());
			resultLookup.put("newSexualPartners",newSexualPartners);
			Result sexualPartner1=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Sexual partner1").within(Duration.days(-1)).last());
			resultLookup.put("sexualPartner1",sexualPartner1);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result newSexualPartners = (Result) resultLookup.get("newSexualPartners");
		Result Input1Value = (Result) resultLookup.get("Input1Value");
		Result sexualPartner1 = (Result) resultLookup.get("sexualPartner1");
		Result Input1 = (Result) resultLookup.get("Input1");
		Result interactedWithSexualPartners = (Result) resultLookup.get("interactedWithSexualPartners");
		...
		if((!interactedWithSexualPartners.isNull()&&interactedWithSexualPartners.toString().equalsIgnoreCase("Yes"))&&
			(!newSexualPartners.isNull()&&newSexualPartners.toString().equalsIgnoreCase("Yes"))&&
			!sexualPartner1.exists()){
			return true;
		}
			return false;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Sexual partner1".length();
				value=userVarMap.get("Sexual partner1");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Sexual partner1".endsWith("_value"))
				{
					variable = "Sexual partner1".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Sexual partner1".endsWith("_date"))
				{
					variable = "Sexual partner1".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Sexual partner1") != null){
						value = resultLookup.get("Sexual partner1").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Sexual partner1");
				}
	...
	public void initAction() {
		...
		actions.add("Please add sexual partner interacted with since last survey?");
	}
