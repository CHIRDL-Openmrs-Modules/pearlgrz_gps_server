/********************************************************************
 Translated from - tookPrescriptionDrugsNotOwned.mlm on Thu Mar 18 15:02:18 EDT 2010
...
public class tookPrescriptionDrugsNotOwned implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify drugs related problems. , , singleChoice";
	}
	...
	public String getTitle(){
		return "Took prescription drugs not owned Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result tookPrescriptionDrugsNotOwned=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Took prescription drugs not prescribed to you").within(Duration.days(-1)).last());
			resultLookup.put("tookPrescriptionDrugsNotOwned",tookPrescriptionDrugsNotOwned);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result tookPrescriptionDrugsNotOwned = (Result) resultLookup.get("tookPrescriptionDrugsNotOwned");
		...
		if(tookPrescriptionDrugsNotOwned.exists()){
			return false;
		}
			return true;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Took prescription drugs not prescribed to you".length();
				value=userVarMap.get("Took prescription drugs not prescribed to you");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Took prescription drugs not prescribed to you".endsWith("_value"))
				{
					variable = "Took prescription drugs not prescribed to you".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Took prescription drugs not prescribed to you".endsWith("_date"))
				{
					variable = "Took prescription drugs not prescribed to you".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Took prescription drugs not prescribed to you") != null){
						value = resultLookup.get("Took prescription drugs not prescribed to you").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Took prescription drugs not prescribed to you");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you take prescription drugs not prescribed to you?");
		actions.add("¿Sabía usted toma medicamentos recetados que no han sido recetados a usted?@Spanish");
	}
