/********************************************************************
 Translated from - wentToMall.mlm on Thu Mar 18 11:22:21 EDT 2010
 ...
 Validation : research
 Purpose:  Identify activity related problems.
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class wentToMall implements Rule, DssRule{
	...
	public String getDate(){
		return "2010-02-04T16:12:57-0500";
	}
	...
	public String getPurpose(){
		return "Identify activity related problems.";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Went to mall".length();
				value=userVarMap.get("Went to mall");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Went to mall".endsWith("_value"))
				{
					variable = "Went to mall".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Went to mall".endsWith("_date"))
				{
					variable = "Went to mall".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Went to mall") != null){
						value = resultLookup.get("Went to mall").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Went to mall");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you go to mall today?");
		actions.add("¿Vas a ir al centro comercial de hoy?@Spanish");
	}
