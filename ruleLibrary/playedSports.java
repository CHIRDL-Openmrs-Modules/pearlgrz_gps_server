/********************************************************************
 Translated from - playedSports.mlm on Thu Mar 18 11:22:30 EDT 2010
 ...
 Validation : research
 Purpose:  Identify sports related problems.
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class playedSports implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify sports related problems.";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Played sports".length();
				value=userVarMap.get("Played sports");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Played sports".endsWith("_value"))
				{
					variable = "Played sports".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Played sports".endsWith("_date"))
				{
					variable = "Played sports".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Played sports") != null){
						value = resultLookup.get("Played sports").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Played sports");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you play sports?");
		actions.add("¿Sabía usted practicar deportes?@Spanish");
	}
