/********************************************************************
 Translated from - addSexualPartner2.mlm on Fri Mar 19 18:27:21 EDT 2010

 Title:  sexual partner related Question
 Filename:  addSexualPartner2.mlm
 Version: 1.0
...
public class addSexualPartner2 implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result sexualPartner2=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Sexual partner1").within(Duration.days(-1)).last());
			resultLookup.put("sexualPartner2",sexualPartner2);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result sexualPartner2 = (Result) resultLookup.get("sexualPartner2");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Sexual partner2".length();
				value=userVarMap.get("Sexual partner2");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Sexual partner2".endsWith("_value"))
				{
					variable = "Sexual partner2".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Sexual partner2".endsWith("_date"))
				{
					variable = "Sexual partner2".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Sexual partner2") != null){
						value = resultLookup.get("Sexual partner2").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Sexual partner2");
				}
