/********************************************************************
 Translated from - wentToMovie.mlm on Thu Mar 18 00:02:20 EDT 2010
...
public class wentToMovie implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify sports related problems. , , singleChoice";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result wentToMovie=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Went to a movie").within(Duration.days(-1)).last());
			resultLookup.put("wentToMovie",wentToMovie);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result wentToMovie = (Result) resultLookup.get("wentToMovie");
		...
		if((!wentToMovie.isNull()&&wentToMovie.toString().equalsIgnoreCase("Yes"))){
			return false;
		}
			return true;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Went to a movie".length();
				value=userVarMap.get("Went to a movie");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Went to a movie".endsWith("_value"))
				{
					variable = "Went to a movie".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Went to a movie".endsWith("_date"))
				{
					variable = "Went to a movie".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Went to a movie") != null){
						value = resultLookup.get("Went to a movie").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Went to a movie");
				}
				varLen = "Yes".length();
