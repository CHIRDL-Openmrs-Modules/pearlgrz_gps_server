/********************************************************************
 Translated from - removeGeneralPartners.mlm on Fri Mar 19 18:37:18 EDT 2010

 Title:  general partner related Question
 Filename:  removeGeneralPartners.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  remove general partners. , , multiSelect , , partnerOptions
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class removeGeneralPartners implements Rule, DssRule{
	...
	public String getPurpose(){
		return "remove general partners. , , multiSelect , , partnerOptions";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result removeGeneralPartners=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Remove general partners").within(Duration.days(-1)).last());
			resultLookup.put("removeGeneralPartners",removeGeneralPartners);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result removeGeneralPartners = (Result) resultLookup.get("removeGeneralPartners");
		...
		if(removeGeneralPartners.exists()){
			return false;
		}
			return true;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Remove general partners".length();
				value=userVarMap.get("Remove general partners");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Remove general partners".endsWith("_value"))
				{
					variable = "Remove general partners".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Remove general partners".endsWith("_date"))
				{
					variable = "Remove general partners".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Remove general partners") != null){
						value = resultLookup.get("Remove general partners").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Remove general partners");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Please select non-sexual partners to remove from history?");
	}
