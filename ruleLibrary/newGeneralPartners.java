/********************************************************************
 Translated from - newGeneralPartners.mlm on Fri Mar 19 18:27:22 EDT 2010

 Title:  general partner Question
 Filename:  newGeneralPartners.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  Identify new general partner related problems. , , singleChoice
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class newGeneralPartners implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify new general partner related problems. , , singleChoice";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "New general partners to add".length();
				value=userVarMap.get("New general partners to add");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("New general partners to add".endsWith("_value"))
				{
					variable = "New general partners to add".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("New general partners to add".endsWith("_date"))
				{
					variable = "New general partners to add".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("New general partners to add") != null){
						value = resultLookup.get("New general partners to add").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","New general partners to add");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Want to add new non-sexual partners since last survey?");
	}
