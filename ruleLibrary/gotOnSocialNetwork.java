/********************************************************************
 Translated from - gotOnSocicalNetwork.mlm on Sun Mar 07 01:37:17 EST 2010
 ...
 Validation :
 Purpose:  Identify interests related problems
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class gotOnSocialNetwork implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify interests related problems";
	}
	...
	public String getTitle(){
		return "social networking Question";
	}
	...
	public String getLogic(){
		return "If conclude endif If If CALL endif If CALL endif endif";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Got on a social networking site, Facebook, MySpace, or other sites".length();
				value=userVarMap.get("Got on a social networking site, Facebook, MySpace, or other sites");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Got on a social networking site, Facebook, MySpace, or other sites".endsWith("_value"))
				{
					variable = "Got on a social networking site, Facebook, MySpace, or other sites".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Got on a social networking site, Facebook, MySpace, or other sites".endsWith("_date"))
				{
					variable = "Got on a social networking site, Facebook, MySpace, or other sites".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Got on a social networking site, Facebook, MySpace, or other sites") != null){
						value = resultLookup.get("Got on a social networking site, Facebook, MySpace, or other sites").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Got on a social networking site, Facebook, MySpace, or other sites");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Got on a social networking site, Facebook, MySpace, or other sites?");
	}
