/********************************************************************
 Translated from - watchedTVOrMovie.mlm on Thu Mar 18 11:22:25 EDT 2010
...
public class watchedTVOrMovie implements Rule, DssRule{
	...
	public String getTitle(){
		return "watch TV or movie Question";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Watched TV or a movie".length();
				value=userVarMap.get("Watched TV or a movie");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Watched TV or a movie".endsWith("_value"))
				{
					variable = "Watched TV or a movie".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Watched TV or a movie".endsWith("_date"))
				{
					variable = "Watched TV or a movie".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Watched TV or a movie") != null){
						value = resultLookup.get("Watched TV or a movie").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Watched TV or a movie");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Watched TV or a movie?");
	}
