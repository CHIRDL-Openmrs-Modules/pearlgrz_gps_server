/********************************************************************
 Translated from - hangoutWithFamily.mlm on Thu Mar 18 15:02:20 EDT 2010
...
public class hangoutWithFamily implements Rule, DssRule{
	...
	public String getTitle(){
		return "hang out with family Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result hangoutWithFamily=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Hang out with family").within(Duration.days(-1)).last());
			resultLookup.put("hangoutWithFamily",hangoutWithFamily);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result hangoutWithFamily = (Result) resultLookup.get("hangoutWithFamily");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Hang out with family".length();
				value=userVarMap.get("Hang out with family");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Hang out with family".endsWith("_value"))
				{
					variable = "Hang out with family".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Hang out with family".endsWith("_date"))
				{
					variable = "Hang out with family".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Hang out with family") != null){
						value = resultLookup.get("Hang out with family").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Hang out with family");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you hang out with family?");
		actions.add("¿Sabía usted salir con la familia?@Spanish");
	}
