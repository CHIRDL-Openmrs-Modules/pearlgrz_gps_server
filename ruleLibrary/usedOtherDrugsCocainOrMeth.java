/********************************************************************
 Translated from - usedOtherDrugsCocainOrMeth.mlm on Thu Mar 18 15:02:21 EDT 2010
...
public class usedOtherDrugsCocainOrMeth implements Rule, DssRule{
	...
	public String getTitle(){
		return "Drugs Cocain or Meth Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result usedOtherDrugsCocainOrMeth=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Used other drugs cocain or meth").within(Duration.days(-1)).last());
			resultLookup.put("usedOtherDrugsCocainOrMeth",usedOtherDrugsCocainOrMeth);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result usedOtherDrugsCocainOrMeth = (Result) resultLookup.get("usedOtherDrugsCocainOrMeth");
		...
		if(usedOtherDrugsCocainOrMeth.exists()){
			return false;
		}
			return true;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Used other drugs cocain or meth".length();
				value=userVarMap.get("Used other drugs cocain or meth");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Used other drugs cocain or meth".endsWith("_value"))
				{
					variable = "Used other drugs cocain or meth".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Used other drugs cocain or meth".endsWith("_date"))
				{
					variable = "Used other drugs cocain or meth".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Used other drugs cocain or meth") != null){
						value = resultLookup.get("Used other drugs cocain or meth").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Used other drugs cocain or meth");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you use other drugs: Cocain or Meth?");
		actions.add("¿Ha utilizado otras drogas: cocaína o metanfetamina?@Spanish");
	}
