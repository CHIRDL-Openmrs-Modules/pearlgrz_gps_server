/********************************************************************
 Translated from - didHomework.mlm on Thu Mar 18 15:02:17 EDT 2010

 Title:  Did homework Question
 Filename:  didHomework.mlm
 Version: 1.0
...
public class didHomework implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result didHomework=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Did homework").within(Duration.days(-1)).last());
			resultLookup.put("didHomework",didHomework);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result didHomework = (Result) resultLookup.get("didHomework");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Did homework".length();
				value=userVarMap.get("Did homework");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Did homework".endsWith("_value"))
				{
					variable = "Did homework".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Did homework".endsWith("_date"))
				{
					variable = "Did homework".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Did homework") != null){
						value = resultLookup.get("Did homework").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Did homework");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you do homework?");
		actions.add("¿Hizo la tarea?@Spanish");
	}
