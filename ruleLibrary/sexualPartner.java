/********************************************************************
 Translated from - sexualPartner.mlm on Thu Mar 18 23:32:23 EDT 2010
...
public class sexualPartner implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify sexual partner related problems. , , singleChoice";
	}
	...
	public String getTitle(){
		return "sexual partner Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result sexualPartner=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt").within(Duration.days(-1)).last());
			resultLookup.put("sexualPartner",sexualPartner);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result sexualPartner = (Result) resultLookup.get("sexualPartner");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt".length();
				value=userVarMap.get("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt".endsWith("_value"))
				{
					variable = "Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt".endsWith("_date"))
				{
					variable = "Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt") != null){
						value = resultLookup.get("Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Interacted with someone that subject had sex or is interested in having sex with since last scheduled prompt");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you interact with someone you had sex or are interested in having sex since last survey?");
	}
