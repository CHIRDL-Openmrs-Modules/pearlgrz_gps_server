/********************************************************************
 Translated from - smokedCigarattes.mlm on Thu Mar 18 15:02:17 EDT 2010
...
public class smokedCigarattes implements Rule, DssRule{
	...
	public String getTitle(){
		return "smoke cigarettes Question";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result smokedCigarattes = (Result) resultLookup.get("smokedCigarattes");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Smoked cigarettes".length();
				value=userVarMap.get("Smoked cigarettes");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Smoked cigarettes".endsWith("_value"))
				{
					variable = "Smoked cigarettes".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Smoked cigarettes".endsWith("_date"))
				{
					variable = "Smoked cigarettes".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Smoked cigarettes") != null){
						value = resultLookup.get("Smoked cigarettes").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Smoked cigarettes");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you smoke cigarettes?");
		actions.add("Fumaste cigarrillos?@Spanish");
	}
