/********************************************************************
 Translated from - generalPartner.mlm on Fri Mar 19 18:27:21 EDT 2010
...
public class generalPartner implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify general partner related problems. , , singleChoice";
	}
	...
	public String getTitle(){
		return "general partner Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result generalPartner=context.read(
			...
			resultLookup.put("generalPartner",generalPartner);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result generalPartner = (Result) resultLookup.get("generalPartner");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Interacted with someone that subject is interested in since last scheduled prompt".length();
				value=userVarMap.get("Interacted with someone that subject is interested in since last scheduled prompt");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Interacted with someone that subject is interested in since last scheduled prompt".endsWith("_value"))
				{
					variable = "Interacted with someone that subject is interested in since last scheduled prompt".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Interacted with someone that subject is interested in since last scheduled prompt".endsWith("_date"))
				{
					variable = "Interacted with someone that subject is interested in since last scheduled prompt".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Interacted with someone that subject is interested in since last scheduled prompt") != null){
						value = resultLookup.get("Interacted with someone that subject is interested in since last scheduled prompt").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Interacted with someone that subject is interested in since last scheduled prompt");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you interact with someone you are talking to or interested in since last survey?");
	}
