/********************************************************************
 Translated from - hangoutWithFriends.mlm on Thu Mar 18 15:02:19 EDT 2010

 Title:  hange out with friends Question
 Filename:  hangoutWithFriends.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  Identify school related problems.
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class hangoutWithFriends implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify school related problems.";
	}
	...
	public String getTitle(){
		return "hange out with friends Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result hangoutWithFriends=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Hang out with friends").within(Duration.days(-1)).last());
			resultLookup.put("hangoutWithFriends",hangoutWithFriends);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result hangoutWithFriends = (Result) resultLookup.get("hangoutWithFriends");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Hang out with friends".length();
				value=userVarMap.get("Hang out with friends");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Hang out with friends".endsWith("_value"))
				{
					variable = "Hang out with friends".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Hang out with friends".endsWith("_date"))
				{
					variable = "Hang out with friends".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Hang out with friends") != null){
						value = resultLookup.get("Hang out with friends").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Hang out with friends");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you hang out with friends?");
		actions.add("¿Sabía usted pasar el rato con los amigos?@Spanish");
	}
