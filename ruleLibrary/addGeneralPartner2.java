/********************************************************************
 Translated from - addGeneralPartner2.mlm on Fri Mar 19 18:27:25 EDT 2010

 Title:  general partner related Question
 Filename:  addGeneralPartner2.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  add new general partners. , , singleInput
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class addGeneralPartner2 implements Rule, DssRule{
	...
	public String getDate(){
		return "2010-03-18T16:12:57-0500";
	}
	...
	public String getPurpose(){
		return "add new general partners. , , singleInput";
	}
	...
	public String getTitle(){
		return "general partner related Question";
	}
	...
	public String getData(){
		return "read read read If read read endif";
	}

	/*** @see org.openmrs.module.dss.DssRule#getLogic()*/
	public String getLogic(){
		return "If If conclude conclude endif If If CALL endif endif";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			resultLookup.put("Input1Value",Input1Value);		if((!mode.isNull()&&mode.toString().equalsIgnoreCase("PRODUCE"))){

			Result interactedWithGeneralPartners=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Interacted with someone that subject is interested in since last scheduled prompt").within(Duration.days(-1)).last());
			resultLookup.put("interactedWithGeneralPartners",interactedWithGeneralPartners);
			Result newGeneralPartners=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("New general partners to add").within(Duration.days(-1)).last());
			resultLookup.put("newGeneralPartners",newGeneralPartners);
			Result generalPartner2=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("General partner1").within(Duration.days(-1)).last());
			resultLookup.put("generalPartner2",generalPartner2);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result interactedWithGeneralPartners = (Result) resultLookup.get("interactedWithGeneralPartners");
		Result Input1 = (Result) resultLookup.get("Input1");
		Result newGeneralPartners = (Result) resultLookup.get("newGeneralPartners");
		Result generalPartner2 = (Result) resultLookup.get("generalPartner2");
		...
		if((!interactedWithGeneralPartners.isNull()&&interactedWithGeneralPartners.toString().equalsIgnoreCase("Yes"))&&
			(!newGeneralPartners.isNull()&&newGeneralPartners.toString().equalsIgnoreCase("Yes"))&&
			!generalPartner2.exists()){
			return true;
		}
			return false;
		}
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "General partner2".length();
				value=userVarMap.get("General partner2");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("General partner2".endsWith("_value"))
				{
					variable = "General partner2".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("General partner2".endsWith("_date"))
				{
					variable = "General partner2".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("General partner2") != null){
						value = resultLookup.get("General partner2").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","General partner2");
				}
	...
	public void initAction() {
		...
		actions.add("Please add non-sexual partner interacted with since last survey?");
	}
