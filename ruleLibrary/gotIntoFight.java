/********************************************************************
 Translated from - gotIntoFight.mlm on Thu Mar 18 11:22:18 EDT 2010
 ...
 Validation : research
 Purpose:  Identify alcohol related problems.
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class gotIntoFight implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify alcohol related problems.";
	}
	...
	public String getTitle(){
		return "fighting related Question";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Got into a fight".length();
				value=userVarMap.get("Got into a fight");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Got into a fight".endsWith("_value"))
				{
					variable = "Got into a fight".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Got into a fight".endsWith("_date"))
				{
					variable = "Got into a fight".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Got into a fight") != null){
						value = resultLookup.get("Got into a fight").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Got into a fight");
				}
				varLen = "Yes".length();
