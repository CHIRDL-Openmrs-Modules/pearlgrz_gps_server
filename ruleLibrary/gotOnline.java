/********************************************************************
 Translated from - gotOnline.mlm on Thu Mar 18 11:22:30 EDT 2010

 Title:  online surfing related Question
 Filename:  gotOnline.mlm
 Version: 1.0
...
public class gotOnline implements Rule, DssRule{
	...
	public String getTitle(){
		return "online surfing related Question";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Got online".length();
				value=userVarMap.get("Got online");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Got online".endsWith("_value"))
				{
					variable = "Got online".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Got online".endsWith("_date"))
				{
					variable = "Got online".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Got online") != null){
						value = resultLookup.get("Got online").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Got online");
				}
				varLen = "Yes".length();
