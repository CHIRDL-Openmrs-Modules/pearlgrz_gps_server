/********************************************************************
 Translated from - tookaNap.mlm on Thu Mar 18 15:02:21 EDT 2010

 Title:  Took a nap Question
 Filename:  tookaNap.mlm
 Version: 1.0
...
public class tookaNap implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result tookaNap=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Took a nap").within(Duration.days(-1)).last());
			resultLookup.put("tookaNap",tookaNap);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result tookaNap = (Result) resultLookup.get("tookaNap");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Took a nap".length();
				value=userVarMap.get("Took a nap");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Took a nap".endsWith("_value"))
				{
					variable = "Took a nap".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Took a nap".endsWith("_date"))
				{
					variable = "Took a nap".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Took a nap") != null){
						value = resultLookup.get("Took a nap").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Took a nap");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you take a nap?");
		actions.add("¿Sabía usted tomar una siesta?@Spanish");
	}
