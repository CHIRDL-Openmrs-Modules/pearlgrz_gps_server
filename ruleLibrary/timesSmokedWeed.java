/********************************************************************
 Translated from - timesSmokedWeed.mlm on Thu Mar 18 14:57:17 EDT 2010

 Title:  smoking weed related Question
 Filename:  timesSmokedWeed.mlm
 Version: 1.0
 ...
 Validation : research
 Purpose:  Identify smoking weed related problems. , , singleSelect , , frequencyOptions
 Explanation: 
 Keywords: 
 Citations: 
 Links: 
...
public class timesSmokedWeed implements Rule, DssRule{
	...
	public String getPurpose(){
		return "Identify smoking weed related problems. , , singleSelect , , frequencyOptions";
	}
	...
	public String getTitle(){
		return "smoking weed related Question";
	}
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Number of times smoking weed since last survey".length();
				value=userVarMap.get("Number of times smoking weed since last survey");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Number of times smoking weed since last survey".endsWith("_value"))
				{
					variable = "Number of times smoking weed since last survey".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Number of times smoking weed since last survey".endsWith("_date"))
				{
					variable = "Number of times smoking weed since last survey".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Number of times smoking weed since last survey") != null){
						value = resultLookup.get("Number of times smoking weed since last survey").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Number of times smoking weed since last survey");
				}
	...
	public void initAction() {
		...
		actions.add("Number of times smoked weed?");
	}
