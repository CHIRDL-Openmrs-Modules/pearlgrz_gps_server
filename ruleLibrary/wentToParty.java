/********************************************************************
 Translated from - wentToParty.mlm on Thu Mar 18 11:22:29 EDT 2010
...
public class wentToParty implements Rule, DssRule{
...
return false;
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		if((!Box1.isNull()&&Box1.toString().equalsIgnoreCase("true"))){
				varLen = "Went to a party".length();
				value=userVarMap.get("Went to a party");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Went to a party".endsWith("_value"))
				{
					variable = "Went to a party".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Went to a party".endsWith("_date"))
				{
					variable = "Went to a party".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Went to a party") != null){
						value = resultLookup.get("Went to a party").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Went to a party");
				}
				varLen = "Yes".length();
