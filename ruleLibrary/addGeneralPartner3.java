/********************************************************************
 Translated from - addGeneralPartner3.mlm on Fri Mar 19 18:27:26 EDT 2010

 Title:  general partner related Question
 Filename:  addGeneralPartner3.mlm
 Version: 1.0
...
public class addGeneralPartner3 implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result generalPartner3=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("General partner3").within(Duration.days(-1)).last());
			resultLookup.put("generalPartner3",generalPartner3);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result generalPartner3 = (Result) resultLookup.get("generalPartner3");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "General partner3".length();
				value=userVarMap.get("General partner3");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("General partner3".endsWith("_value"))
				{
					variable = "General partner3".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("General partner3".endsWith("_date"))
				{
					variable = "General partner3".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("General partner3") != null){
						value = resultLookup.get("General partner3").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","General partner3");
				}
