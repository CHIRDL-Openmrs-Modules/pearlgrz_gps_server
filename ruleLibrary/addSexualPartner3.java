/********************************************************************
 Translated from - addSexualPartner3.mlm on Fri Mar 19 18:27:17 EDT 2010

 Title:  sexual partner related Question
 Filename:  addSexualPartner3.mlm
 Version: 1.0
...
public class addSexualPartner3 implements Rule, DssRule{
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result sexualPartner3=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Sexual partner3").within(Duration.days(-1)).last());
			resultLookup.put("sexualPartner3",sexualPartner3);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result sexualPartner3 = (Result) resultLookup.get("sexualPartner3");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Sexual partner3".length();
				value=userVarMap.get("Sexual partner3");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Sexual partner3".endsWith("_value"))
				{
					variable = "Sexual partner3".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Sexual partner3".endsWith("_date"))
				{
					variable = "Sexual partner3".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Sexual partner3") != null){
						value = resultLookup.get("Sexual partner3").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Sexual partner3");
				}
