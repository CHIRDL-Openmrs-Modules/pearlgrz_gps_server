/********************************************************************
 Translated from - wenttoschool.mlm on Thu Mar 18 15:02:20 EDT 2010
...
public class wenttoschool implements Rule, DssRule{
	...
	public String getTitle(){
		return "Went to School Question";
	}
...
return false;
	...
	public Result eval(LogicContext context, Patient patient,
		...
		try {
			...
			Result wentToSchool=context.read(
				patient,context.getLogicDataSource("obs"),
				new LogicCriteria("Went to school").within(Duration.days(-1)).last());
			resultLookup.put("wentToSchool",wentToSchool);}
	...
	private boolean evaluate_logic(Map<String, Object> parameters) throws LogicException {
		...
		Result wentToSchool = (Result) resultLookup.get("wentToSchool");
		...
		if((!Input1.isNull()&&Input1.toString().equalsIgnoreCase("true"))){
				varLen = "Went to school".length();
				value=userVarMap.get("Went to school");
				if(value != null){
					parameters.put("param1",value);
				}
				// It must be a result value or date
				else if("Went to school".endsWith("_value"))
				{
					variable = "Went to school".substring(0, varLen-6); // -6 for _value
					if (resultLookup.get(variable) != null){
						value = resultLookup.get(variable).toString();
					}
				}
				else if("Went to school".endsWith("_date"))
				{
					variable = "Went to school".substring(0, varLen-5); // -5 for _date
					if (resultLookup.get(variable) != null){
				...
				{
					if (resultLookup.get("Went to school") != null){
						value = resultLookup.get("Went to school").toString();
					}
				}
				if(value != null){
					parameters.put("param1",value);
				...
				{
					parameters.put("param1","Went to school");
				}
				varLen = "Yes".length();
	...
	public void initAction() {
		...
		actions.add("Did you go to school today?");
		actions.add("¿Vas a ir a la escuela hoy?@Spanish");
	}
